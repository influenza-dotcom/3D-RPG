...
func test_opposed_factions_are_mutually_hostile() -> void:
	var raider: RangedEnemy = load(RANGED_PATH).new()
	var towns: RangedEnemy = load(RANGED_PATH).new()
...
func test_same_faction_ignore_each_other() -> void:
	var a: RangedEnemy = load(RANGED_PATH).new()
	var b: RangedEnemy = load(RANGED_PATH).new()
...
func test_factioned_vs_unaligned_never_fights() -> void:
	var raider: RangedEnemy = load(RANGED_PATH).new()
	var loner: RangedEnemy = load(RANGED_PATH).new()
	raider.faction = _faction(&"raiders", { &"townsfolk": -1.0 })
