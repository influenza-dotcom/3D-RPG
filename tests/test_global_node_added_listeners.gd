...
## B-F5/F57 invariant: SceneTree.node_added is a GLOBAL signal fired for EVERY node that enters the tree. TWO game
## subsystems connect to it — star_sky (sky FX when a WorldEnvironment enters) and menu_style (button SFX for
## BaseButtons under menu roots) — both with cheap early-outs. A THIRD listener taxes EVERY node instantiation
