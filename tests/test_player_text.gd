...
func test_screen_composer_registry() -> void:
	# Stats summary: six-space column gaps are the readout's look (the middle dots are gone).
	assert_eq(PlayerText.stats_summary(3, 12.5, 0), "Level 3      12.5 zorkmids",
		"summary without spare points — level + wallet only")
	assert_eq(PlayerText.stats_summary(3, 12.5, 1), "Level 3      12.5 zorkmids      1 perk point",
		"singular perk-point tail selects its whole template")
	assert_eq(PlayerText.stats_summary(10, 1500.0, 3), "Level 10      1500 zorkmids      3 perk points",
	...
	assert_eq(PlayerText.journal_objective("Kill the boss", true, 1, 1, false), "Kill the boss",
		"done -> checked box")
	assert_eq(PlayerText.journal_objective("Collect parts", false, 2, 5, false), "Collect parts (2/5)",
		"a counted objective shows progress")
	assert_eq(PlayerText.journal_objective("Collect parts", true, 5, 5, false), "Collect parts (5/5)",
		"counted + done")
	...
	assert_eq(PlayerText.journal_objective("Find the cache", true, 1, 1, true), "Find the cache  (optional)",
		"optional + done")
	assert_eq(PlayerText.journal_objective("Collect parts", false, 2, 5, true), "Collect parts (2/5)  (optional)",
		"counted + optional")
	assert_eq(PlayerText.journal_objective("Collect parts", true, 5, 5, true), "Collect parts (5/5)  (optional)",
