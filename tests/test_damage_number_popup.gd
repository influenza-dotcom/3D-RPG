...
func test_text_for_rounds_real_loss_and_keeps_tiny_visible_hits_at_one() -> void:
	assert_eq(DamageNumberPopup.text_for(12.4), "12", "damage numbers round the actual HP loss")
	assert_eq(DamageNumberPopup.text_for(12.6), "13", "larger fractions round up")
	assert_eq(DamageNumberPopup.text_for(0.2), "1", "a displayed hit never shows 0 damage")
...
func test_should_show_requires_player_shooter_non_player_character_and_real_loss() -> void:
	...
	assert_true(DamageNumberPopup.should_show(victim, 5.0, player),
		"the human player's shots on non-player Characters get damage-number feedback")
	assert_false(DamageNumberPopup.should_show(victim, DamageNumberPopup.MIN_LOSS - 0.01, player),
		"sub-threshold chip hits stay quiet")
	assert_false(DamageNumberPopup.should_show(victim, 5.0, npc_shooter),
		"NPC, ally, or companion firefights don't spawn the player's damage numbers")

	victim.add_to_group(&"Player")
	assert_false(DamageNumberPopup.should_show(victim, 5.0, player),
...
func test_throwable_impacts_route_real_loss_into_damage_numbers() -> void:
	...
	assert_true(src.contains("DamageNumberPopup.show(character, real_loss, global_position, false, attacker)"),
