...
## resting at the sides — and NOT as the first-person arms rig that fists.tres carries as its `view_model`. A real weapon
...
func test_fists_fallback_reads_as_unarmed() -> void:
	...
	assert_true(InspectScreen._is_unarmed_fallback(Player.FISTS),
		"the FISTS bare-hands fallback must show as unarmed in the inspect showcase (no mounted view model, arms down)")
...
func test_loaded_fists_by_path_reads_as_unarmed() -> void:
	...
	# ever handed back a duplicate wouldn't leak the arms rig back into the 'unarmed' hero-view.
