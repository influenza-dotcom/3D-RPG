...
## PlayerStats — the RPG stat sheet and its effect formulas, plus the seams that consume them.
## The load-bearing contract: a BASELINE sheet is perfectly NEUTRAL (all multipliers 1.0, all bonuses 0),
## so adding the stat system changed nothing until a stat is authored off baseline.
...
func test_get_stat_by_name_for_dialogue_checks() -> void:
	...
	assert_eq(s.get_stat(&"no_such_stat"), PlayerStats.BASELINE,
