...
func test_baseline_sheet_is_perfectly_neutral() -> void:
	...
	assert_almost_eq(s.weapon_damage_mult(), 1.0, 0.0001, "baseline gunplay changes no weapon damage")
...
func test_stat_formulas_move_the_right_direction() -> void:
	...
	assert_almost_eq(s.weapon_damage_mult(), 1.25, 0.0001, "gunplay 5 -> +25% weapon damage (PD-1 gunner build)")
	assert_almost_eq(s.headshot_damage_bonus(), 1.25, 0.0001, "gunplay 5 -> +25% headshot punch (PD-1)")
