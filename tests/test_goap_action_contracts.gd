...
func test_action_costs_are_pinned() -> void:
	...
	assert_almost_eq(GoapActionInvestigate.new().base_cost, 0.2, 0.0001, "Investigate = 0.2 (distinct from Detect)")
...
func test_action_names_are_pinned() -> void:
	...
	assert_eq(GoapActionInvestigate.new().name, &"Investigate")
...
func test_action_preconditions_are_pinned() -> void:
	...
	_assert_dict(GoapActionInvestigate.new().preconditions, {&"state_investigating": true}, "Investigate pre")
...
func test_action_effects_are_the_sentinel_goal_facts() -> void:
	...
	_assert_dict(GoapActionInvestigate.new().effects, {&"spot_searched": true}, "Investigate eff")
