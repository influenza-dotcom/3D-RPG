...
## ONE-READ-PER-FILE cache shared by the audit panel's four domain scanners for the duration of a single Re-scan.
##
## WHY: a Re-scan runs scan_disk (which also runs scan_wiring), scan_text and scan_menu_sound, and every one of them
...
##
## CONTRACT: audit_panel wraps its scan in begin() ... end(). Between those, text_of(path) returns the cached text,
## reading from disk only the first time each path is asked for. OUTSIDE that window the cache is INERT — text_of
...
## only ever alive inside one synchronous _rescan(), so it cannot outlive an edit: end() drops it unconditionally.

static var _texts: Dictionary = {}
static var _active := false


## Open a caching window. Called by audit_panel._rescan before the domain scanners run. Idempotent — a second
