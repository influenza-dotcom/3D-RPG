...
## Array[{severity, source, message, node}] -- `node` is the offending Node ref so the panel can jump to it.
...
static func run(root: Node) -> Array:
	...
	for n in SceneWalk.collect_all(root):
		...
		for w in SceneWalk.config_warnings(n):
			out.append({"severity": "WARN", "source": src, "message": str(w), "node": n})
		...
		if n is NavigationRegion3D:
			...
			if nm == null or nm.get_vertices().size() == 0:
				out.append({"severity": "ERROR", "source": src, "message": "NavigationRegion3D isn't baked (0 navmesh vertices) — NPCs have nothing to path on.", "node": n})
		# Typed: duplicate non-blank PlayerSpawn entry_id (GameRoot uses the FIRST match).
		if n is PlayerSpawn:
			var id: StringName = (n as PlayerSpawn).entry_id
			if id != &"":
				if spawn_ids.has(id):
					out.append({"severity": "WARN", "source": src, "message": "Duplicate PlayerSpawn entry_id '%s' — GameRoot uses the FIRST, so the rest are dead." % id, "node": n})
				spawn_ids[id] = true
	return out
