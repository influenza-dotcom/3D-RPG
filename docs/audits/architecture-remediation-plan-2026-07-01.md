...
> **M12 sign-off — RESOLVED OUT-OF-SCOPE (task owner, 2026-07-03).** M12 was one of the original 37 ids, but the review provided **no verified fix spec** and explicitly marked its fix *optional*. The task owner has **confirmed M12 out-of-scope**: the four `Engine.time_scale` writers (`freeze_frame`, `bullet_time`, the death cinematic, `GameState` quickload) coordinate by a *deliberate, tested convention* — `bullet_time.gd`'s `_managing_time_scale` latch restores only if it lowered the scale, and it explicitly defers to the death cinematic — which already encodes the freeze/death > bullet-time priority the arbiter would centralize. No bug forces the change; the arbiter is architectural polish carrying real refactor risk on playtest-only timing code. Revisit ONLY if a concrete time-scale conflict surfaces (which would also supply the spec's requirements). With this sign-off, **all 37 finding ids are resolved** — the plan is scope-complete.
>
