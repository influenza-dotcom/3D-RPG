# Stealth System — Improvement & Expansion Plan
...
**GOAP** (`scripts/npc/goap/`, `use_goap` default false): `GoapExecutor._build_world_state(host)` snapshots `Perception.state` into 3 bool facts `state_detecting/alerted/investigating` + `threat_noticed` (`goap_executor.gd:73-92`). Goals are `GoapGoal` (data: `desired_state` dict + `base_priority`); the planner is A* with an **integer `unmet_count` heuristic** (`goap_goal.gd:25-30`, `goap_planner.gd:24,45`) and `key()` dedup. Actions subclass `GoapAction` and re-check live perception in `is_runtime_valid`. Per-archetype authoring is `GoapProfile.tres`. The FSM mirror `_fsm_tick()` (`npc.gd:1457-1489`) is byte-for-byte fallback; `GoapActionInvestigate` is a verbatim transcription of its INVESTIGATING arm.
...
**Designer-first idiom:** `GameSettings` autoload `preload()`s `resources/tuning/*.tres` (`managers/GameSettings.gd:96-110`). Keybinds register in `managers/InputManager.gd` + `project.godot [input]` + `OptionsMenu.REBINDABLE` (`scripts/ui/options_menu.gd:30-45`, which already lists Crouch/Throw/Light/NightVision). Player-facing tunables persist via the `Settings` autoload.
...
- **GOAP + FSM:** On the GOAP path, once `hear_only()` flips `state` to INVESTIGATING, `_build_world_state` already reads `_perception.state` → `state_investigating`/`threat_noticed` flip → existing Investigate goal runs. **But** the executor only ticks with a valid `_target` (`npc.gd:1448` is past the no-target return), so the idle-time hearing must ALSO drive a minimal investigate from the no-target branch (mirror the FSM INVESTIGATING arm there, or bind a transient non-enemy "point of interest" target so the seam runs). FSM path: the no-target idle branch calls `hear_only()` then routes to the INVESTIGATING behaviour.
...
- **GOAP + FSM:** `_build_world_state` sets `&"detection"` (float) and `&"suspicion_tier"` (int) as **read-only priority facts only**. **Hard invariant (document next to the SENTINEL-FACTS note at `goap_executor.gd:69`):** these must NEVER appear in a goal `desired_state` — the A* heuristic is integer `unmet_count` with `key()` dedup (`goap_goal.gd:25`, `goap_planner.gd:24,45`); a continuous float in `desired_state` would explode the search and break admissibility. FSM path gets the feature for free (HUD reads the meter; `_fsm_tick` unchanged).
...
- **GOAP + FSM:** No new fact/goal/action — it changes how fast `&"detection"` rises, consumed via Slice 1. Lives inside Perception, below both brains → both paths get it free.
...
- **GOAP + FSM:** Feeds the same `&"detection"` fact via the falloff multiplier — no planner change. Optional `&"target_in_shadow"` bool for a future "flush them out" action (not required v1). Automatic on both paths.
...
- **GOAP + FSM:** None — it only changes `noise_radius`, consumed by existing hearing.
- **Effort:** S · **Risk:** Low.
...
- **GOAP + FSM:** Once `hear_only()`/`sense()` flips `state` to INVESTIGATING at the decoy point, the existing Investigate goal/action and the FSM INVESTIGATING arm walk the NPC there. Optional `&"heard_decoy"` lets a wary archetype prioritize Investigate harder. Mirror the no-target investigate from Slice 0a.
...
- **GOAP + FSM:** `_build_world_state` sets `&"corpse_in_view"` (LOS-checked nearest corpse) + its position fact; new `GoapActionCheckBody` (precond `corpse_in_view`, effect `body_checked`) walks there and calls `_perception.alert_to(corpse_pos)`; new `GoapGoal('CheckBody')` between Investigate and Idle. FSM: a new branch in `_fsm_tick`'s UNAWARE arm that, on seeing a corpse, calls `alert_to(corpse.global_position)` and barks via `NpcVoice`.
...
- **GOAP + FSM:** NPC side is reactive — it dies via the existing damage path; `off_guard` already read in `DamageApplier`. No GOAP/FSM branch for the math. The takedown feeds Slice 5 (leaves a body) and must trip the suppression rule there.
...
- **GOAP + FSM:** Only changes whether `can_hear()` returns true and where `last_known_position` lands — both paths consume it already. No new fact/goal/action; lives inside Perception.
...
- **GOAP + FSM (heaviest dual-path tax — verified):** `GoapActionInvestigate.act` is transcribed **verbatim** from `_fsm_tick`'s INVESTIGATING arm, so changing the search touches **both** copies. Recommend authoring this **GOAP-first** as `GoapActionSearch` (precond `state_investigating`, effect `spot_searched`, expose `&"search_progress"` 0..1) and mirroring **minimally** into `_fsm_tick` — accept that FSM code as deleted at Phase 4 rather than maintaining byte-for-byte parity while Phase 3 goal migration is still in flight. A/B against the FSM before any per-archetype `use_goap` flip.
...
- This is Slice 1 (+ 0b). Touches no perception/targeting logic, no GOAP/FSM edits, and surfaces the `StealthStatus` signature change early while it's cheap.

**SHIP-2 — sensory depth, all inside Perception, below both brains:** Slice 2 (distance/angle falloff), then Slice 3 (light/shadow) and the cheap Slice 3b (slow-walk). All default to behaviour-preserving values; unit-test `visibility_factor(dist, angle, light)` as a pure function. Needs Slice 0b done.
...
**SHIP-5 — consequence + occlusion:** Slice 5 (body discovery — new dedicated `Corpse` marker, `GoapActionCheckBody` + FSM branch, the suppression rule binding it to Slice 6), then Slice 7 (sound occlusion, makes Slice 4 tactical).

**SHIP-6 — capstone:** Slice 8 (search nuance), GOAP-first + minimally mirrored, A/B before any `use_goap` flip.

**CROSS-CUTTING RULE:** because `use_goap` defaults false, every *behaviour* slice (0a, 4, 5, 8) must land on BOTH the GOAP path AND `_fsm_tick` in the same change — exactly as the current Detect/Investigate actions mirror the FSM arms. *Sensory* slices (2, 3, 7) live inside Perception and get both paths for free. All new tunables = `@export` (via the Slice 0b reachability path) or a `*Settings.tres` on `GameSettings`; the Takedown keybind + the HUD toggle wire into InputManager/project.godot/OptionsMenu+Settings per CLAUDE.md.
...
**Risks:**
- **Dual-path tax is real and ~2× per behaviour slice.** `GoapActionInvestigate` is already a verbatim FSM transcription; Slices 0a/4/5/8 each add a fact/goal/action AND a mirrored `_fsm_tick` branch while Phase 3 migration is mid-flight. Budget the A/B gate per slice. For Slice 8, prefer GOAP-first + minimal mirror over byte-for-byte parity (the FSM copy is deleted at Phase 4 anyway).
...
- Rebuild Perception, NoiseEmitter, or the FSM — every slice extends existing seams.
- Flip `use_goap` to true or accelerate the Phase 3/4 GOAP cutover — that is gated on the manual-playtest A/B, not on this work.
