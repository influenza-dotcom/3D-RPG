...
1. **Open a level.** Open `res://scenes/TestLevel.tscn` (or `TestLevel_2.tscn`) from the FileSystem dock. This is a playground scene with ground, navigation, and a player spawn already wired -- a safe place to drop things. (The game itself boots at `res://computerroom.tscn` — the computer-room intro that hosts the start menu — set as the main scene.)
...
The game's main scene is `res://computerroom.tscn` (set in `rpg/project.godot` under `run/main_scene`), so pressing **F5 (Run Project)** boots the computer-room intro — click to power the monitor on and the start menu (`res://scenes/start_menu.tscn`, instanced over the room by `computerroom.gd`) fades in. While iterating on a specific level it's faster to open that level's `.tscn` in the editor and press **F6 (Run Current Scene)** — that launches the level you're looking at directly, skipping the menu.
...
The 21 tabs group into four jobs.
...
- **Dialogue Edit** — pick a `DialogueResource` and edit the conversation: each line + its choices (target line, a
...
- **Quest Edit** — title / description / reward money+XP / `prereq_quest_id` + `next_quest` chaining + the objective
  list (typed KILL/TALK/PICKUP/… dropdown, target, count), add / remove / reorder, then **Save**. (Item `rewards[]`
  and `reward_reputation` stay inspector-only.)
...
- **Play-from-spawn toolbar** — **▶ Play** runs the main scene (`res://computerroom.tscn`); **▶ Spawn** runs it starting the player at the
...
- The **shipped catalog is `res://resources/characters/PlayerAppearanceCatalog.tres`** — edit it in the Inspector to add/tune heads, bodies, palettes. (A code fallback, `CharacterAppearanceCatalog.default()`, is used only if that `.tres` is ever missing, so the customizer never hard-breaks.) `get_catalog()` prefers the `.tres`.
...
- **Heads** compose onto a torso body. Each head needs its OWN `scale` / `position` / `rotation` — a different model won't fit the `headblue` seat (`scale` `0.205`, `position` `(0, 0.615, 0.04)`, `rotation` `(0, 90, 0)`). **Tune a head exactly like an NPC:** open **`res://scenes/character_customizer_setup.tscn`** (a `BodyModelSwap` on the shipped torso), set its `head_model` to the head, dial `head_scale` / `head_position` / `head_rotation` live in the 3D viewport, then copy those three values into that head's `CharacterPartOption` in the `.tres`. The four shipped heads (`headblue` / `head` / `female` / `chrysalis`) currently share the `headblue` seat as a starting point — `headblue` fits; the other three still want per-head tuning this way.
...
- **Colours**: `skin` tints the head + body, `arm` / `leg` tint the limbs, and (in first person) the player's own view-model legs + carry-arms pick up the chosen `leg` / `arm` colour too. `WHITE` = "no tint / natural" (show the model/texture as authored, same sentinel as `NpcLook`) — it leads the `skin_palette` on purpose but is deliberately kept OUT of `limb_palette`. Edit `skin_palette` / `limb_palette` (`PackedColorArray`s on the catalog) to change the offered swatches.
...
- **`-1` = END (the default for `target_on_fail`)** — a botched skill check ends the conversation.
...
Like every other consequence, these fire **on PICK regardless of a gate's pass/fail** — if you want the rep hit only on a *failed* skill check, route `target_on_fail` to a line whose choice carries the write.
...
Relevant files: `res://scripts/components/passive_item_buffs.gd` (`class_name PassiveItemBuffs`), `item.gd` (`held_passive_effect` / `passive_unique`), the fold hooks `Character.status_stat_modifier()` / `status_move_multiplier()`, and the strength factors `CharacterStats.CARRY_PER_STRENGTH` / `HP_PER_STRENGTH` / `MELEE_DAMAGE_PER_STRENGTH`.
...
**Exports**
- `grants` (PackedScene) — **the preferred way.** Drag an ability scene from `res://scenes/components/abilities/` here (the shipped set: `Grapple.tscn`, `LaserSight.tscn`, `WallClimb.tscn`, `AirDash.tscn`, `Slide.tscn`, `FallImmunity.tscn`, `ChessVisualizer.tscn` — the Board Visualizer chip for the blindfold-chess minigame). Its node — with its own authored config — is added under the player on pickup. Takes precedence over `unlock_id`. A scene whose root isn't an `Ability` is discarded and grants nothing (fails safe).
- `unlock_id` (String) — **fallback**, used only when `grants` is empty. It's an `ENUM_SUGGESTION` dropdown offering `grapple, laser_sight, wall_climb, air_dash, slide, fall_immunity, chess_visualizer` (you can pick from the list or leave it blank); the chosen id is passed to `player.unlock_mechanic()`. Prefer `grants`. The dropdown **self-populates** from the `.tscn` files in `scenes/components/abilities/` via `AbilityRegistry` (each scene's PascalCase filename snake-cased), so it can't drift out of sync with the shipped abilities.
...
- **`UpgradePickup`** (`upgrade_pickup.gd`) — permanently grants a player ability. Drag an ability scene from `scenes/components/abilities/*.tscn` into `grants`; set `display_name`, `toast_color`, optional `world_model`. (`unlock_id` is a string fallback whose dropdown — `grapple,laser_sight,wall_climb,air_dash,slide,fall_immunity` — self-populates from that abilities folder via `AbilityRegistry`, so it never drifts.) Builds a glowing emblem when you leave the body unauthored. Also `save_id` / `persist_collected` (like `MoneyPickUp`) so a granted upgrade never re-grants on Continue.
...
| `takedown` | `SilentTakedownSettings.tres` | The silent-takedown verb (hold the Takedown key behind an off-guard NPC): `enabled` (master switch), `hold_time` (`3.0` s held to commit), `max_range` (`1.5` m melee reach), `behind_arc_degrees` (`120°` rear arc — matches `DamageApplier.is_behind`), plus the `require_behind` (`true`) / `require_crouch` (`true`) gates (the shipped `SilentTakedownSettings.tres` sets `hold_time = 3.0`, `max_range = 1.5`, and `require_crouch = true`, overriding the gentler script defaults) |
...
- **Stats** (`strength` … `agility`) are bought one point at a time for zorkmids at a **`LevelUp`** station (below). "Total level" there is just the *sum of your seven stats* (= points purchased).
...
Drop a **`LevelUp`** under a trainer or shrine. The dialogue adds "Level Up" when the speaker has a child with `level_up_stat` + `level_up_cost`; it suspends the conversation and opens `LevelUpScreen`, where the player spends zorkmids to raise any one of `strength`, `gunplay`, `agility`, `streetwise`, `stealth`, or `pickpocket` by 1.

- **`station_name: String`** — hover + screen title; blank → "Level Up".
- **`base_cost: int`** (default `1`) — cost at total level 0.
- **`cost_per_level: float`** (default `1.5`) — added per point already invested, so cost climbs Dark-Souls style with your total level (the sum of all seven stats). Formula: `base_cost + (total_level × cost_per_level)`.
- **`standalone: bool`** — `true` for a self-serve shrine; `false` under a dialogue NPC.

**Flat, Dark-Souls cost — the same for every stat.** Cost depends **only** on total level, not on which stat you buy or how high it already is: raising a maxed stat costs exactly what raising a fresh one does. There's no per-stat "specialization tax" — the old `cost_per_stat_point` export is **gone**. One curve, identical across all seven stats.
...
- **`stat_bonuses` keys must be exact `CharacterStats` names.** `strength` / `gunplay` / `agility` / `streetwise` / `stealth` / `pickpocket` only — any other key (a typo, a made-up stat) is dropped and grants nothing, but `Perk.validate()` now `push_warning`s it on unlock, so check the console if a perk does less than you intended.
...
A shop you clean out or a crate you loot stays empty forever unless you say otherwise. Drop a **`Restocker`** (`rpg/scripts/components/restocker.gd`, `@tool`, plain `Node`) as a **child of the `Merchant` / `ItemContainer`** (or point `target_path` at one) and it refills the host back up to its fixed authored stock/item rows.
...
- **Perk `stat_bonuses` keys** — recursively scans `res://resources/` for `Perk` resources and flags any whose `stat_bonuses` has a key that isn't a real `CharacterStats` attribute (`strength`/`gunplay`/`agility`/`streetwise`/`stealth`/`pickpocket`) — i.e. a typo that would silently grant nothing (§13).
...
| `stealth` | enemy detection meter fills 5% slower (harder to spot) | §17 Perception detection fill |
| `pickpocket` | lower chance an NPC catches you lifting an item, and a bigger value you can lift unnoticed | §8 loot / pickpocket flow |
...
Every multiplier is exactly `1.0` at baseline `0`, so an unsheeted or all-baseline character's damage is unchanged — the scaling only bites once you push the stat off `0`. It applies to any `Character` with the sheet, so a strength-heavy brawler NPC or a `gunplay`-heavy gunner NPC hits harder too. Per-point conversion constants live on `CharacterStats` (`MELEE_DAMAGE_PER_STRENGTH`, `WEAPON_DAMAGE_PER_GUNPLAY`, `HEADSHOT_PER_GUNPLAY`, `SWAY_PER_GUNPLAY`, `PRICE_PER_STREETWISE`, `REP_PER_STREETWISE`, `MOVE_PER_AGILITY`, `JUMP_PER_AGILITY`, `DETECTION_PER_STEALTH`, and the stamped `CARRY_PER_STRENGTH` / `HP_PER_STRENGTH` / `STAMINA_PER_ENDURANCE`).

#### `pickpocket` and the `PickpocketSettings` resource

**`pickpocket`** is read at the loot screen's pickpocket flow (§8): each point lowers the chance an NPC catches you lifting an item **and** raises how valuable a thing you can steal unnoticed (and, past a threshold, eventually the weapon in their hands). The tunables live on a global resource **`PickpocketSettings`** (`GameSettings.pickpocket`, `res://resources/tuning/PickpocketSettings.tres`, §12):

- **`base_catch_chance`** (`0.35`) — the catch chance at `0` pickpocket.
...
- **`equipped_pickpocket_threshold`** (`8`) — the pickpocket level at which lifting the weapon *in an NPC's hands* becomes possible.
...
Key files: `rpg/scripts/player/character.gd` (`max_hp` + the `stats` slot), `rpg/scripts/player/character_stats.gd` (the seven attributes + their formulas), `rpg/resources/tuning/EconomySettings.gd` (`player_starting_money`).
...
**Menus don't shift with text.** Every screen is built so no runtime string (a merchant/station name, a cost, a stat value, a money total) can change an element's width, position, or alignment. Two disciplines enforce this, and any new menu code must follow them:
- **Full-panel screens** (inventory / shop / loot / stats / level-up / options) pin their `PanelContainer` to a fixed screen *fraction* (`anchor_left = 0.12 … anchor_bottom = 0.88`, offsets `0`), so the panel is content-independent. Value columns use fixed `custom_minimum_size.x` (right-aligned), and unbounded labels are capped with `MenuStyle.cap_label()` (`clip_text` + `…` -- clip_text is what actually drops a Label's min width so the cap holds; ellipsis alone does not in a content-sized parent).
