...
**How well an NPC shoots is its `stats` sheet (2026-08-25).** NPCs are no longer aimbots: every ranged shot an NPC fires deflects by a random **aim-error cone**, and the cone is the species-wide base (`GameSettings.npc_ai` → **Targeting & engagement** → `aim_error_deg`, default `2.5°`) scaled by the archetype's **gunplay** stat through the *same* steadiness formula the player's aim wander uses (`CharacterStats.sway_mult` — 8% tighter per point over baseline, perfectly accurate at 12.5+). So accuracy is authored as **identity**: give the archetype's `stats` sub-resource a `gunplay` value and it shoots like who it is — a scared shopkeeper with `gunplay -4` sprays 32% wider, `sniper.tres`'s authored `gunplay 2` shoots 16% tighter, an elite bodyguard at `gunplay 8` is nearly surgical. (The same sheet already scales its ranged *damage* — PD-1 — so one number moves both.) For scale: at the 2.5° base, a stationary player ~10 m out is roughly a coin flip per bullet; point-blank shots still land. This stacks with, and is distinct from, `miss_chance` (a per-NPC roll for a *deliberate* wide warning shot) — and note every enemy round is now a **live projectile** flying at `projectile_speed × npc_projectile_speed_mult`, so movement dodges what the cone doesn't (see *Authoring a WeaponData*). Set `aim_error_deg` to `0` to restore perfect NPC aim globally.
...
- **General** — `effective_range` (metres; for the **player** this is where the hitscan ray stops, and a ranged NPC uses it as its standoff distance; `0` = unranged. ⭐ **For an AI wielder it caps nothing any more**: since 2026-08-25 *enemies never hitscan* — every ranged AI shot is a **live projectile** (`ShotResolver.ai_fires_live_projectile`) whose damage travels with the round, so `effective_range` only shapes where the NPC *stands and pulls the trigger*. An NPC with a `projectile_scene` and a positive `effective_range` also **fires while still closing**, up to `GameSettings.npc_ai.fire_grace_range` metres beyond the standoff; an *unranged* lob like the rock gets no such band, since its fallback engage distance already exceeds its ballistic reach), and `move_speed_multiplier`, the speed scale applied to the wielder *while this weapon is drawn* (`1.0` = no penalty; the SMG uses `0.93`, fists use `1.25` to make you faster bare-handed).
...
- **Projectile** — assign a `projectile_scene` (a Bullet scene, e.g. `res://scenes/projectiles/Projectile.tscn`) for a travelling round; leave it unset for pure hitscan (player-only — **an AI needs one on every ranged weapon or it deals NO damage at all**, since enemies never hitscan; every shipped gun has one, and melee is the deliberate exception). Then tune `projectile_speed`, `projectile_life_time`, `bullet_gravity_scale` (`0` = flat laser, `1` = grenade arc), `launch_angle`, and `has_tracer`.
  - **`npc_projectile_speed_mult`** (default `1.0`) — the **AI dodge-window dial**: multiplies `projectile_speed` *only* when an NPC fires this weapon, so enemy rounds fly visibly slower than yours from the identical gun (the classic AI-fairness trick) without touching the player's feel. **`pistol.tres`, `smg.tres` and `shotgun.tres` author `0.5`** — at 40 m/s an enemy pistol round takes a readable quarter-second to cross 10 m, which is what makes strafing a real defense. Leave it `1.0` on the sniper (its terror is the fast bolt; the charge/beep telegraph is the counterplay) and on ballistic lobs like the rock (the arc is tuned to its speed). ⭐ Because AI damage lives entirely on the round, `projectile_speed × npc_projectile_speed_mult × projectile_life_time` must reach at least `effective_range + npc_ai.fire_grace_range`, or max-range shots despawn mid-air — `tests/test_combat_data.gd` (`test_shipped_ai_rounds_outfly_the_attempt_band`) ratchets every shipped `.tres` (the sniper shipped exactly that bug: 500 m flight vs a 508 m attempt band).
  - **GOTCHA — a projectile scene MUST scan physics layer 1, or it flies through the level.** The map's brush geometry (func_godot `worldspawn` / `func_geo` / `func_detail`, see `addons/func_godot/fgd/*.tres`) is authored **`collision_layer = 1`, `collision_mask = 0`** — one `StaticBody3D` for the whole world that **scans nothing** and relies entirely on whatever hits it to scan layer 1. Characters (`Player.tscn`, `enemy.tscn`) are `collision_layer = 2`. So a projectile root needs **`collision_layer = 0`, `collision_mask = 3`** — the setting on `Projectile.tscn` and `sphere_projectile.tscn`. Mask out layer 1 and the round silently passes through every wall, floor and ceiling, then dies on its `life_time` timeout — a path that deliberately does **not** emit `queued_for_deletion`, so `explosion.gd`'s bridge never fires and no blast, decal or impact sound ever happens. It still detonates on people (layer 2), which makes the symptom read as "it only explodes on NPCs". `rock_projectile.tscn` shipped `collision_mask = 2` and had exactly that bug; `tests/test_rocket_world_impact.gd` now flies a real round into a worldspawn-shaped slab and pins it. ⭐ Since AI damage is projectile-only, a mis-masked scene now zeroes an enemy weapon's **entire damage against the world-side of any fight** (cover, floors, blast surfaces) instead of merely breaking its visuals — the mask is load-bearing.
