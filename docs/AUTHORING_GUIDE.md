...
To go further: add a third line for a real branch — make line 1's `"LOVE IT!"` choice `target = 2`, and author line 2 as the enthusiastic follow-up. To add a skill gate, give a choice `required_stat = &"streetwise"` and `required_value = 6`; it'll show `[Streetwise 6] ...` and lock until the player's streetwise clears 6.
