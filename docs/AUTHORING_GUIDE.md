...
- A new **NPC** → instance `res://scenes/enemies/NPC.tscn` (an armed combatant), `res://scenes/enemies/civilian.tscn` (a non-combat townsperson — townsfolk faction, flees threats, pre-wired with a `Talkable`), or `res://scenes/enemies/medicine_person.tscn` under `Characters`, then set its `@export` fields in the inspector (`display_name`, `faction`, `profile`, `weapon_data`, `wanders`, `sight_range`, …). Drop a `talkable.tscn` instance under the NPC to make it conversational (set its `dialogue` and `voice`).
