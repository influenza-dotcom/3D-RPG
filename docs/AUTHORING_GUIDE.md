...
A level is a scene (`.tscn`) under `rpg/scenes/`. You'll see existing levels like `rpg/scenes/TestLevel.tscn` and `rpg/scenes/TestLevel_2.tscn`, plus building blocks in `scenes/world/`, `scenes/props/`, `scenes/enemies/`, and `scenes/weapons/`. The flow is always the same: **drop a component node** (surface 1), **set its `@export` fields** (surface 2), and where it asks for content, **assign a `.tres`** (surface 3). Behaviour is composed from nodes, never hidden in a monolithic script â€” so you can add, remove, or rearrange features just by editing the scene tree.
...
- **`rpg/scripts/ui/options_menu.gd`** â€” the `OptionsMenu` overlay (autoload, code-built, opened with Escape; serves both the start menu and in-game). It's organised into tabs, rebuilt each open by `_rebuild_tabs()` in this on-screen order: `_build_video_tab()`, `_build_audio_tab()`, `_build_spotify_tab()`, `_build_game_tab()`, `_build_controls_tab()`, `_build_accessibility_tab()`. You add your control to the matching tab with a one-line helper â€” `_slider_row(...)`, `_check_row(...)`, or `_option_row(...)` â€” pointing its setter at your new `Settings.set_â€¦` method. Edits stage into `_pending` and only commit on **Apply** (key rebinds are the exception â€” they bind live).

**Keybinds** follow the same "wire it everywhere" rule, with one extra stop. To add a rebindable action: register it in `managers/InputManager.gd` **and** in `project.godot`'s `[input]` map, **then** add an entry to the `OptionsMenu.REBINDABLE` array in `options_menu.gd` (an `{"action": &"MyAction", "label": "My Action"}` dict, under one of the `{"section": â€¦}` headers â€” Movement / Combat / Interface / Hotbar). The action *name* is the stable key â€” rebinding only swaps the bound event, so everything that polls the action name keeps working. `Settings.rebind_action()` persists the new event under the cfg's `[controls]` section.
...
- **Player-facing tunables must be wired into the Options menu *and* `Settings`.** A new tunable (volume, sensitivity, FOV, accessibility, screen-shake, â€¦) isn't done when the gameplay code reads it â€” add a typed `var` + a `set_*` setter to the **`Settings`** autoload (`res://managers/Settings.gd`) and ONE `SettingSpec` row to `res://resources/settings/SettingsCatalog.tres` (the Options menu is data-driven — no `options_menu.gd` edits). New keybinds additionally go in `InputManager.gd` + `project.godot [input]` + a `Keybind` row in that catalog.
