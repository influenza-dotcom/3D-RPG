...
| `effects` | `EffectsSettings.tres` | Visual FX: decal fade/placement, dust puffs, the on-screen blood overlay, gore gibs & world blood drops, the **Body-part gibs** group (`body_part_gibs_enabled` and the `body_part_gib_*` feel knobs — the burst that flings a dying character's own head/torso/arms/legs), the enemy **Death freeze** beat (`death_freeze_duration` — how long an enemy holds its pose before bursting into gore), explosion visuals, sky/hit flashes, the **Muzzle & Impact FX** group (`muzzle_flash_radius`, `hit_spark_backoff`, `hit_spark_speed_to_scale`, `overkill_burst_radius`), plus the first-person **Gun Holster (view model)** put-away/draw animation (`gun_holster_animation_time` / `gun_holster_position_offset` / `gun_holster_rotation_offset`) |
...
The cinematic has its own soundtrack, owned by **`DeathMix`** (`rpg/scripts/player/death_mix.gd`, a code-built child of the Player). The world ducks out from under a clip that keeps playing at full level -- so the sting is alone over the black screen and the death card, and is still ringing as the world fades back in around you.

With the shipped 7.97 s clip, one death runs:

```
0.00s  killed          world starts draining, vignette starts closing, camera keels over
0.20s  STING STARTS    a slight offset, so it doesn't land on the same frame as the killing blow
1.60s  full black      world silent under the sting; GameState.player_died fires; card fades in
2.20s  card up         "You were killed by ..." holds  (hold = 4.02s, DERIVED - see below)
6.22s  card fades out
6.82s  black, no text
7.17s  respawn         world starts fading up -- UNDER a sting that is still playing
8.17s  sting ends      on its own, 1.0s into the new life. Nothing faded it.
9.67s  world full      fade-up-from-black complete
```

**The sting is never faded or stopped** -- its own decay is the fade. A forced ramp to silence is perceptually over long before it mathematically finishes, so it amputates the clip's tail and reads as the audio being *cut*; the clip finishing itself, a beat into the new life, does not.

The `4.02 s` hold is not authored anywhere -- `respawn_delay` is still `1.0`. The cinematic solves for it each death so the respawn lands exactly `death_sting_overlap` before the clip ends:

```
death_sequence_time + fade_in + HOLD + fade_out + death_card_gap  ==  sting_end - death_sting_overlap
```

That is what puts the game back *underneath* a sting that is still finishing, rather than after one that already stopped (a black screen waiting on silence) or by cutting one short. It only ever *lengthens* the hold -- a short clip, or no clip, plays the original snappy cinematic unchanged.
...
- **`death_card_holds_for_sting`** (default **on**) -- holds the death card up long enough that the sting plays out and then **cross-fades** with the world coming back, rather than being cut off mid-phrase. `respawn_delay` becomes a **minimum**, and the extra hold is **derived from the clip's real length every death** -- swap `death_sting` for something longer or shorter and the card follows, with no number here to go stale. Turn it off to run the short cinematic on `respawn_delay` alone.
...
- **`death_sting_overlap`** (`1.0` s) -- how long the sting is still sounding **after** you respawn. This is what `death_card_holds_for_sting` solves the card's hold against, so raising it hands more of the clip to live gameplay and lowering it toward `0` lands the clip's end on the respawn itself.
- **`death_sting_release`** (`0` = **off**, the default) -- an *optional* forced fade-out at the respawn. Leave it at `0` and the clip rings out naturally, which is what stops the audio sounding cut. Set a value only for a clip that genuinely outstays its welcome, and keep it at least as long as `death_sting_overlap` or it will chop the tail it was meant to smooth. The `RELOAD_*` death modes always cut the sting dead regardless (a fresh scene, nothing to ring into).
