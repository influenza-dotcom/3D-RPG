...
Also `@export`s on the **`BodyModelSwap` child**, runtime-only. The head-bob and mouth both need a **head node** — a swapped `head_model`, or a legacy `head_scene` (with neither, they're silent no-ops; see Gotchas).
...
- **Keep exactly one `BodyModelSwap` (one `body_model`) per NPC.** It hides the default `Man.glb` body/head and the runtime head-look + sniper-glint retarget to the swapped head. If you set `head_model`, **clear the NPC's own `head_scene`** — the component owns the head then.
