...
- **Player-facing tunables must be wired into the Options menu *and* `Settings`.** A new tunable (volume, sensitivity, FOV, accessibility, screen-shake, â€¦) isn't done when the gameplay code reads it â€” expose it on the matching `OptionsMenu` tab (`res://scripts/ui/options_menu.gd`) and persist it through the **`Settings`** autoload (`res://managers/Settings.gd`). New keybinds additionally go in `InputManager.gd` + `project.godot [input]` + `OptionsMenu.REBINDABLE`.
