...
- **The player's own HP glow doesn't feed the meter -- keep it tagged that way.** `Player.tscn` ships an always-on `OmniLight3D` child named `PlayerEmittingLight` at the player's own origin (the HP health glow), and `PlayerLightLevel` auto-collects *every* `Light3D` in the scene -- at distance `0` that one's contribution alone would saturate the sum and pin `light_exposure` at `1.0`. So the node ships tagged into the **`&"stealth_light_exempt"`** group, which the meter skips in both discovery modes (the same rule as the `&"pickup_beacon"` item glows) -- darkness works out of the box. Tag ANY decorative light into that group (Node dock -> Groups) to hide it from stealth without curating `&"lights"`; if you re-author or replace `PlayerEmittingLight`, keep the tag or darkness dies again.
