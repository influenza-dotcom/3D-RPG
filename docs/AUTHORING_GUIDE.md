...
- **Audit** — *Re-scan* lists findings errors-first from a scene walk (every node's config warnings, unbaked
  ...
  WARN naming the line — player copy belongs in `PlayerText` (`scripts/ui/player_text.gd`), and
  `tests/test_player_text.gd` holds a shrink-only baseline over the same scanner so the count can only go down.
