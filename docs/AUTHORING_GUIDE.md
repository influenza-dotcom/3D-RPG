...
**Contract notes for anyone editing this.** `deposit()` / `withdraw()` are the only writers of `GameState.account` outside the interest posting, they self-guard and run off-tree, and `AtmScreen` is a pure view that decides nothing — which is why the whole terminal is unit-tested without a scene (`rpg/tests/test_atm.gd`). The screen sounds the machine through the shared static seam `StationSpeaker.chirp(atm)` — type-agnostic and null-safe, so no station screen duck-types a method name and none can drift out of sync with the others.
...
**A new Ability with live state should override `on_deactivated()`** (`scripts/components/abilities/ability.gd`) to end whatever it has in flight — the hook fires when the player switches the implant off, so a later switch-on can't resume stale state. `Slide` ends the slide, `Grapple` severs the rope; a pure gate (air dash, laser sight, fall immunity) needs nothing.
...
- **`bus`** (`&"speaker"`) — **this field is the tinny sound.** Point it at `sfx` for a clean, full-range, expensive-sounding machine. A bus name that doesn't exist falls back to `sfx` with a warning rather than landing on Master, where no volume slider would reach it.
- **`max_distance`** (10 m) / **`unit_size`** (4.0) — how far the chirp carries and how fast it falls off. A panel speaker is not a PA; keep both small so it stays *the machine's* sound.
...
| `endurance` | 0.30 | 0.45 | 0.15 | 5 | only effect is stamina pool size, and sustained sprint throughput is identical at every value — it buys *burst length*. Weighted a little above strict honesty because its sinks are the movement chips this screen sells. |
