...
A component is a node with its own `class_name` and a set of `@export` knobs. You drag it into your scene, point it at the thing it should affect, and configure it in the Inspector — no branching logic, no scripting. These live in `rpg/scripts/components/` (and their abilities in `rpg/scripts/components/abilities/`). Examples you'll use constantly: `LookAtInteractable` and its family (the ones that extend it — `CanPickUp`, `MoneyPickUp`, `ItemContainer`, `Merchant`, `LootableCorpse`), plus standalone drop-ins like `Lock`, `CanDestroy`, `SpawnOnDestroy`, `Throwable`, `Healer`, `Bonfire`, `Radio`, and `UpgradePickup`. The abilities (all extending `Ability`) — `Slide`, `WallClimb`, `Grapple`, `AirDash`, `LaserSight`, `FallImmunity`, `SilentTakedownAbility`, and `ChessVisualizer` — drop under the Player. Some ship as ready-made one-node scenes in `rpg/scenes/components/` (e.g. `container.tscn`, `merchant.tscn`, `radio.tscn`, plus the ones in `scenes/components/abilities/`) so you can drag the whole thing in at once.
...
Tick **`sitting`** on the **NPC root** and the `BodyModelSwap` child holds a seated pose while the NPC is idle (any combat/alert state, companion-follow, or cutscene control stands it back up; dialogue keeps it seated and the NPC speaks in place). The pose knobs live on the **`BodyModelSwap` child**:

- **`seated_snap_to_ground`** (bool, default **true**) — at runtime the seated visual **drops until it rests flush on whatever surface is straight below the NPC** (a raycast: the floor for a ground-sitter, the seat for an NPC parked on a chair/crate). You just place the NPC and tick `sitting`; the seat height sorts itself out per spot. The probe re-runs only when the NPC actually moves, so parked sitters cost next to nothing. Off (or nothing within `seated_max_snap_depth` below), it falls back to the fixed `seated_visual_offset`. **The seat needs a real layer-1 collider the NPC's capsule can stand on** — on a decorative collider-less chair the capsule (and so the snap) falls through to the floor, and the NPC sits through the seat.
- **`seated_max_snap_depth`** (float, default `1.2` m) — how far below the NPC's origin the probe reaches; it doubles as a sanity gate, so an NPC whose origin pokes past a ledge rim **rejects** the cliff-bottom hit (keeping the fallback offset) instead of rendering its seated body into the drop.
- **`seated_hip_clearance`** (float, default `0.06` m) — how high the hip joint (`leg_position`) rests above the snapped surface; roughly half the leg's thickness. Trim it until the butt/legs visibly touch rather than float or sink — **in a playtest**: the editor preview never probes, so this knob has no viewport effect.
- **`seated_hand_clearance`** (float, default `0.05` m) — the hands never come closer to the snapped surface than this: the seated arm pitch **auto-raises** past `seated_arm_pitch` (never below it) using the arm model's measured reach. In practice the snap pins the shoulder a fixed height over the seat plane, so a probe-valid sitter's arms always settle at the same clamped angle (**~55° hands-on-lap on the shipped rig**, floor and chair alike) — `seated_arm_pitch` only changes the runtime pose when authored *steeper* than that.
- **`seated_visual_offset`** (Vector3, default `(0, -0.28, 0.04)`) — with the snap on, only X/Z apply as authored; the Y is the **editor-preview / probe-miss fallback** drop.
- **`seated_body_pitch`** (`8.0`°, forward torso lean), **`seated_arm_pitch`** (`-25.0`°, the *preferred* arm hang — see the clamp above), **`seated_leg_pitch`** (`-90.0`°, legs straight out in front; positive would fold them backward through the seat).
...
The editor previews the full seated pose (drop + leg/arm pitches) using the authored fallback offset — the ground snap itself is runtime-only, so expect the in-game seat to sit **lower** than the editor preview when the NPC sits on the floor, and the in-game arms to rest **steeper** (on the lap, per the clamp above) than the previewed `seated_arm_pitch`.
...
- **View Model & cosmetics** — `view_model` is the first-person weapon scene the gun rig instantiates on equip (the SMG points at `ak_472.tscn`, the pistol at `silenced.tscn`); unset = the rig's built-in placeholder shows. Then `has_muzzle_flash`, `has_laser_sight`, `spawns_casing`, `casing_size_scale`, and the **Audio** streams (`audio`, `whiz_sound`, `impact_sound`, `impact_enemy_sound`, `reload_sound`).
...
| `pickpocket` | `PickpocketSettings.tres` | The loot-screen PICKPOCKET flow (crouch-interact an off-guard NPC, §22): the catch roll (`base_catch_chance` `0.35` at larceny 0, lowered by `catch_chance_per_point` `0.03`), the liftable-value ceiling (`base_value_allowance` `10.0` + `value_allowance_per_point` `5.0`), `equipped_pickpocket_threshold` (`8` — the larceny needed to lift the weapon in an NPC's hands), and `caught_witness_radius` (`12.0` m — how far a blown lift rallies nearby enemies to turn and look; `0` = only the victim reacts) |
...
| **Awareness states** | An enemy's perception ladder -- UNAWARE → DETECTING → INVESTIGATING → ALERTED -- surfaced to the player as the detection readout. (§19) |
