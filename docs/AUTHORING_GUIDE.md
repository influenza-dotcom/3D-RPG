...
- **Only `trigger_volume.tscn` ships as a prefab.** `EncounterSpawner`, `WaveManager`, `NpcPool` and `CutscenePlayer` are bare nodes you add yourself. Example RESOURCES do ship, though: `res://resources/encounters/raider_squad.tres` (a `SpawnDefinition`) and `res://resources/cutscenes/sample_cutscene.tres` (a `Cutscene`).
...
- **`sitting` outranks patrol.** Ticking `sitting` on the NPC root makes `_idle` return before it ever reaches `ScheduleBehavior` / `PatrolBehavior` / wander, so a seated guard will never walk its beat — pick one. (Combat/search, companion-follow and cutscene control all stand it back up, at which point the normal order resumes.)
...
- **A seated NPC can only get home by blinking.** The idle walk-back deliberately doesn't stand a sitting NPC up (it just re-faces it), so with `blink_home = false` a shopkeeper dragged off his stool never returns to it. Leave the blink on for seated casts.
...
- **`caliber = &""` (empty)** — no reserve. The clip refills for free on every reload. This is what melee, fists, and spray paint use. Combined with `is_infinite_ammo = true`, melee weapons never run dry.
...
- **Inline:** set the NPC node's **`weapon_data`** export (`@export var weapon_data: WeaponData`, the "Weapon" group) to any `.tres`. `null` = **no GUN** — no weapon hub, laser, or ranged fire path is built, though it still senses, wanders, and faces. It is *not* a non-combatant by itself: with the class defaults (`disposition = HOSTILE`, `threat_response = FIGHT`) it closes to reach and **punches** with `fists.tres` (its `damage`, `enemy_knockback`, `enemy_lift`). For a true civilian townsperson also set Hostility → a NEUTRAL/FRIENDLY disposition or faction and Behavior → `threat_response = FLEE` (see "Reaction: `threat_response`" and "Worked example — a fleeing townsperson"). A non-null value = a **combatant**.
...
- `is_melee = true` (Damage group) — the explicit flag that *makes* it a melee weapon: damage scales with the wielder's STRENGTH instead of gunplay, the gunplay headshot bonus is skipped, and each swing costs melee stamina (`stamina_melee_attack_cost`). It is never inferred from `effective_range`, so leaving it `false` silently gives you a gunplay-scaled "melee" weapon (see "The strength / gunplay split").
- `is_infinite_ammo = true`, `max_ammo = 1`, empty `caliber` → never reloads, never runs dry.
- No `projectile_scene`, `projectile_speed = 0`, small `effective_range` (the knife is `3.0`, fists `1.5`) → the swing is a short **hitscan ray**, so `effective_range` must stay *positive* (a 0-range ray hits nothing) — it's the reach of the swing, not a bullet's travel. No projectile rig needed.
...
- **What's already in Options → Accessibility** — check here before adding a new comfort knob, the row may already exist: Screen Shake, Screen Flashes, Vertex Warp (PS1), Hit Stop, Colorblind Filter, Colorblind-Safe Cues, View Bobbing, Camera Tilt, FOV Effects, Show Detection Meter, Item Lights, Show Weapon, Left-Handed Weapon, Text-to-Speech, Low-HP Heartbeat. (All 15 are `SettingSpec` rows in `SettingsCatalog.tres` with `tab = &"Accessibility"`.)
...
- **The player carries its own light, so darkness currently does nothing.** `Player.tscn` also ships an always-on `OmniLight3D` child named `PlayerEmittingLight` at the player's own origin, and the shipped `PlayerLightLevel` auto-collects *every* `Light3D` in the scene -- including that one, at distance `0`, whose full contribution alone saturates the sum. So `light_exposure` pins at `1.0` and the light curve can't slow detection until that light is excluded: tag it into the skipped `&"pickup_beacon"` group, or set `auto_collect = false` and curate the `&"lights"` group. Treat a code-side exclusion as the real fix; this is the interim warning.
...
| **`CrippleCallout`** | the callouts when a limb of this NPC is crippled | `enabled` (off = silent cripples); `toast_color` (colour of the "Crippled X's arm" toast shown to the player who did it); `self_bark_template` (the actor's own cry, `%s` = the part word -- author `"My %s!"` for the classic callout). The template **ships EMPTY, so the cry is silent** until you fill it, per the project-wide unauthored-speech policy; even then a lethal hit stays quiet (a dying NPC doesn't call out). |
...
- **`death_message_fall`** (String, default `"You hit the ground at [mph] miles per hour."`) -- the card when fall damage lands the killing blow (both a hard landing *and* the max-continuous-fall death). `[mph]` is replaced with the impact speed; a `%d` / `%s` / `%f` is accepted instead. Unlike the three lines below it, this one ships **authored**, not a `[PH]` placeholder. Set it to `""` for no card on a fall.
...
- **The autosave is a single in-place slot** -- overwritten at each milestone. But an explicit **quicksave** (F5 / F9) layers on top (see *Quicksave & manual slots* below), so you DO have a manual bookmark. Three named slots (`save_to_slot` / `load_from_slot`, `SLOT_COUNT` = `3`) exist on the `GameState` API and round-trip in tests, but have **no player-facing save/load screen and no keybind yet** -- today they're a seam for a menu you build (and for the read-only CYBER SUNDAY *Saves* dock), not something a player can reach.
...
**A quick/slot save is also the EXACT-snapshot tier.** On top of the profile it writes a `[world_snapshot]` cfg section (`rpg/scripts/world/world_snapshot.gd`, with its own `SNAPSHOT_VERSION` decoupled from the profile's save version) capturing the live world -- where every authored NPC in the level you saved in was standing (position / yaw / hp), plus every visited level's authored-NPC death ledger -- which `GameRoot.load_level` re-applies once, after the level subtree enters the tree, via `GameState.consume_world_snapshot()`. The lean autosave/Continue profile **never** carries one (`autosave()` nulls it first), and that is the real difference between the two save products. A snapshot written by an unrecognised version is skipped with a warning and the profile still loads on its own. Containers, loot drops and dynamically-spawned (spawner) NPCs are still out of scope.
...
A crate you can shoot apart, a barrel, a gore gib -- any destructible / grabbable physics prop is a **`Throwable`** component (§11 catalogue) driven by a **`ThrowableData`** resource. `ThrowableData` is to a Throwable what `WeaponData` is to a weapon: one Throwable scene reskins into many object types purely by swapping the `.tres`. Author one with **CYBER SUNDAY → `Content` tab → *New Throwable*** (type an id, it writes the `.tres` into `res://resources/interactables/` and opens it), or by hand with **right-click `res://resources/interactables/` → New Resource → `ThrowableData`** (the shipped examples are `wooden_crate.tres` and `gore_gib_data.tres`) — then assign it to the Throwable's data slot.
...
- **Thrown props can nose toward their travel direction.** Turn on `face_travel_when_thrown` (per-instance, or `ThrowableData.face_travel_when_thrown` for every prop of that type) so a *thrown* prop — a knife, spear, bottle — leads with its front and tips over as it arcs. It's THROW-only: a tap-drop, a forced release (death/quickload), and a re-grab never trigger it. It uses the same `face_carrier_rotation_degrees` for the mesh-front correction, and releases back to natural tumbling once the prop slows below `face_travel_min_speed` — `ThrowableData.face_travel_min_speed` defaults `2.0` m/s; the placed `Throwable.face_travel_min_speed` defaults `0`, which *means* "inherit the resource", so set a positive value on the instance to override it. The whole **Throw Pose** group lives on the `ThrowableData` resource too, so a prop type is configured once and reused. Applies to both the pickup throw (Z/E) and a grapple yank-fling.
