...
static func resolve_damage(weapon: WeaponData, was_crit: bool, off_guard: bool, pierce: float, behind: bool = false, stats: CharacterStats = null, gunplay_mod: float = 0.0) -> float:
	if pierce >= 0.0:
		return pierce
	return scaled_damage(weapon.damage, weapon.headshot_multiplier, weapon.sneak_attack_multiplier, was_crit, off_guard, weapon.backstab_multiplier, behind, stats, gunplay_mod)
...
static func scaled_damage(base: float, crit_mult: float, sneak_mult: float, was_crit: bool, off_guard: bool, backstab_mult: float = 1.0, behind: bool = false, stats: CharacterStats = null, gunplay_mod: float = 0.0) -> float:
	var dmg := base \
			* (crit_mult if was_crit else 1.0) \
			* (sneak_mult if off_guard else 1.0) \
			* (backstab_mult if behind else 1.0)
	# PD-1: the SHOOTER's combat stats scale the hit — GUNPLAY raises raw damage, and a crit gets an extra
	# headshot punch. `gunplay_mod` folds in an active gunplay status buff (0.0 = none). DETERMINISTIC (no RNG) so
	# seeded-spread tests are untouched; null / baseline sheet + no buff = 1.0 no-op.
	if stats != null:
