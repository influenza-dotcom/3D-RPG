...
static func resolve_damage(weapon: WeaponData, was_crit: bool, off_guard: bool, pierce: float, behind: bool = false, stats: CharacterStats = null) -> float:
	if pierce >= 0.0:
		return pierce
	return scaled_damage(weapon.damage, weapon.headshot_multiplier, weapon.sneak_attack_multiplier, was_crit, off_guard, weapon.backstab_multiplier, behind, stats)
...
static func scaled_damage(base: float, crit_mult: float, sneak_mult: float, was_crit: bool, off_guard: bool, backstab_mult: float = 1.0, behind: bool = false, stats: CharacterStats = null) -> float:
	var dmg := base \
	...
	# headshot punch. DETERMINISTIC (no RNG) so seeded-spread tests are untouched; null / baseline sheet = 1.0 no-op.
	if stats != null:
		dmg *= stats.weapon_damage_mult()
		if was_crit:
			dmg *= stats.headshot_damage_bonus()
	return dmg
