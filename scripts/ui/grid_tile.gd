...
var _mesh: SubViewportContainer = null  ## an ItemMeshView, present only when the item has a mesh
...
func _draw() -> void:
	...
	if _mesh == null:
		_draw_glyph(r, col)  # no mesh -> a clean category mark instead of a text label
	if _count > 1:
