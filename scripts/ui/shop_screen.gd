...
func open_shop(merchant: Node, player: Node) -> void:
	if _is_open or DialogueManager.is_active() or OptionsMenu.is_open() or InventoryScreen.is_open() or LootScreen.is_open() or HealScreen.is_open() or LevelUpScreen.is_open() or RespecScreen.is_open() or StatsScreen.is_open() or ReputationScreen.is_open():
		return
