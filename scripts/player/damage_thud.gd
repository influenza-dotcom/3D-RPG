...
##
## Code-built child of Character (added in its _ready). Reads the host's damage_thud @export (it stays
## on the root so the editor/.tscn can still set it); the cooldown + base volume are designer-tunable
## on GameSettings.player_feedback; the only state — the cooldown stamp — lives here since nothing
## outside Character reads it by name.
##
...
## Extra dB stacked on top of GameSettings.player_feedback.damage_thud_volume_db to make the hit
## actually punch — the settings field is the designer-facing base, and this boost is the
## felt-loudness bump applied at the play site. +9 dB roughly doubles perceived loudness, lifting the
## deep, slowed thud from a background duck to a heavy impact. Tune to taste.
...
## off it, and gate on ITS group membership.
var _host: Character
