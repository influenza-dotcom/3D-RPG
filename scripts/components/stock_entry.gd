...
## Designers fill Merchant.stock_counts with these, one row per item kind —
## "3 health packs, 20 pistol clips, 2 shotguns" is three entries. Weapons are stocked as one UNIQUE
## instance per count (two shotguns are two distinct objects), like the rest of the loot pipeline.
