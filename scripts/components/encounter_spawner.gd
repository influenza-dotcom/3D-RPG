...
func _spawn_one(def: SpawnDefinition) -> void:
	...
	# Mark EVERY spawner-produced body a DYNAMIC actor (pooled, pool-less default, OR overflow) so the per-level
	# world ledger excludes it — its runtime @-node_path is ephemeral. Authored .tscn NPCs never pass here (stay false).
	if npc != null:
