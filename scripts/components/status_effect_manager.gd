...
## stacking. An EMPTY-id effect ALWAYS stacks (the StatusEffect.id contract: leave the id blank only when you
## want copies to pile up). Give a weapon/consumable effect an id (poison/burn/adrenaline/...) to refresh — that
## also collapses a multi-pellet hit, where each pellet re-applies the SAME id, into ONE refreshed effect.
