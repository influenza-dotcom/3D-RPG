...
## True while an ENABLED ability grants `id`. The GATE consumed (via Player.has_mechanic) by air_dash / laser_sight /
## grapple / fall_immunity / chess_visualizer; wall_climb / slide are driven through the Player's typed refs instead.
...
## The granted (enabled) ability ids — the save system's projection of the live node set. Deduped (two same-id nodes
## count once). Presence+enabled is the single source of truth; there is no parallel persisted flag to drift from it.
