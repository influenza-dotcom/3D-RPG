...
| ⚠ | `assets/audio/music/wander/Santa Monica Theme.mp3` | 7,303,346 | Vampire: The Masquerade – Bloodlines OST, "Santa Monica" (Rik Schaffer; Activision/Paradox) — YouTube rip (ID3 shows the DASH/`Lavf60.16.100` fingerprint), **knowing placeholder**, added 2026-09-18 | `resources/tuning/WanderMusicSettings.tres` (`tracks`) | REPLACE with an original or licensed exploration loop, then DELETE. Clearing `tracks` makes the whole wandering bed inert BY DESIGN (see `assets/audio/music/wander/README.md`), so the purge is a pure resource edit. |
...
| ⚠ | `assets/audio/sfx/liecio-calming-rain-257596.mp3` | Pixabay (id 257596, user `liecio`) — added 2026-09-18 from `Downloads/`, filename kept as downloaded so the provenance survives | `scripts/components/rain_fall.gd` (`DEFAULT_SOUND_PATH`) — the rain bed every `RainFall` plays unless a stream is assigned | TODO | TODO — confirm the Pixabay Content License covers commercial use and whether attribution is required | TODO — the id suggests `https://pixabay.com/sound-effects/` , find the exact page | |
