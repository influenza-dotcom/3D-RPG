...
## to the material's own `color_steps` (16 on the player's overlay, 32 on the CRT wall), so the default option
## reproduces the look the game shipped with exactly. Order = the Video dropdown, coarsest LAST.
const COLOR_QUANTIZE_LEVELS: Array[Vector3] = [
	Vector3.ZERO,               # 0 Authored — the material's own color_steps (the default; today's look)
	...
	Vector3(15, 15, 15),        # 4 12-bit    — RGB444
